...
class Raw_Data_validation:
    ...
    def validationFileNameRaw(self,regex,LengthOfDateStampInFile,LengthOfTimeStampInFile):
        ...
        onlyfiles =  self.gcp.get_allFiles(self.tariningfiles)
                ...
                if (re.match(regex, filename)):
                    ...
                    if len(splitAtDot[1]) == LengthOfDateStampInFile:
                        if len(splitAtDot[2]) == LengthOfTimeStampInFile:
                            self.gcp.copy_blob(self.tariningfiles, filename, self.good_raw, filename)
                            self.logger.log(f, "Valid File name!! File moved to GoodRaw bucket :: %s" % filename)

                        else:
                            self.gcp.copy_blob(self.tariningfiles, filename, self.bad_raw, filename)
